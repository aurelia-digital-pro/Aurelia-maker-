...
class EpisodeProduction:
    ...
    def generate_visual_assets(self) -> list[Path]:
        ...
        for scene in self.scenes:
            ...
            generate_scene_image(scene.index, scene.title, scene.text, path, direction=scene.direction, width=512, height=512)
            assets.append(path)
        ...
        self.visual_manifest_path.write_text(json.dumps({
            "backend": "local-ai",
            "source": "chat",
            "episode_id": self.episode_id,
            "title": self.title,
            "source_text_sha256": hashlib.sha256(self.script_path.read_text(encoding="utf-8").strip().encode("utf-8")).hexdigest(),
            "scenes": [
                {
                    "index": scene.index,
                    "title": scene.title,
