...
class EpisodeProduction:
    ...
    def generate_visual_assets(self) -> list[Path]:
        self._emit("Generating cinematic visual assets with local AI...")
        assets: list[Path] = []
    ...
    def synthesize_narration(self, script_text: str) -> Path:
        self._emit("Synthesizing narration (local TTS)...")
    ...
    def build_subtitles(self, narration_path: Path) -> Path:
        ...
        intro_duration = min(8.0, total_duration)
        body_duration = max(total_duration - intro_duration, 0.1)
    ...
    def render_shots(self, assets: list[Path]) -> list[Path]:
        ...
        for scene, asset in zip(self.scenes, assets[1:]):
            ...
            engine.render_motion(
                ...
                lighting={"brightness": 1.02 + (scene.index % 3) * 0.02, "contrast": 1.06, "saturation": 1.12},
