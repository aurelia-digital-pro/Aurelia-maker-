...
def probe_has_audio(path: str | Path) -> bool:
    path = Path(path)
    result = run_ffprobe([
        "-v", "error", "-select_streams", "a:0", "-show_entries", "stream=index", "-of", "csv=p=0", str(path)
    ])
...
def concat_clips(clips: list[str | Path], output: str | Path) -> Path:
    ...
    if len(clips) == 1:
        output_path.write_bytes(Path(clips[0]).read_bytes())
        return output_path
    ...
    _run([
        ...
        "-i", str(list_file), "-c:v", "libx264", "-preset", "medium", "-crf", "18",
...
def mix_narration_and_music(video: str | Path, narration: str | Path, output: str | Path, music: str | Path | None = None) -> Path:
    ...
    if music and Path(music).exists():
        _run([
            "-y", "-hide_banner", "-loglevel", "error", "-i", str(video), "-i", str(narration), "-i", str(music),
            "-filter_complex", "[1:a]volume=1.0[narr];[2:a]volume=0.18[bgm];[narr][bgm]amix=inputs=2:duration=first:dropout_transition=2[aout]",
            "-map", "0:v:0", "-map", "[aout]", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", str(output_path),
        ])
    else:
        _run([
            "-y", "-hide_banner", "-loglevel", "error", "-i", str(video), "-i", str(narration),
            "-map", "0:v:0", "-map", "1:a:0", "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", str(output_path),
        ])
    return output_path
...
def apply_color_grade(source: str | Path, output: str | Path, contrast: float = 1.08, saturation: float = 1.12, brightness: float = -0.02) -> Path:
    ...
    _run([
        ...
        "-c:v", "libx264", "-preset", "medium", "-crf", "17", "-c:a", "copy", "-pix_fmt", "yuv420p", str(output_path),
    ])
    return output_path
...
def burn_subtitles(source: str | Path, srt: str | Path, output: str | Path) -> Path:
    ...
    escaped_srt = srt_path.as_posix().replace(":", r"\:")
    subtitle_filter = (
        f"subtitles=filename='{escaped_srt}':"
        "force_style='FontSize=28,PrimaryColour=&HFFFFFF,OutlineColour=&H000000,Outline=2,MarginV=40'"
    )
    _run([
        "-y", "-hide_banner", "-loglevel", "error", "-i", str(source),
        "-vf", subtitle_filter, "-c:v", "libx264", "-preset", "medium", "-crf", "17",
...
def master_encode(source: str | Path, output: str | Path, width: int = 1920, height: int = 1080, fps: int = 24, profile: str = "youtube") -> Path:
    ...
    _run([
        ...
        "-c:v", "libx264", "-preset", "slow", "-crf", "16", "-c:a", "aac", "-b:a", "192k", "-ar", "48000",
...
def generate_ambient_music(duration_sec: float, output: str | Path) -> Path:
    ...
    base = Sine(55).to_audio_segment(duration=duration_ms).apply_gain(-22)
    layer = Sine(110).to_audio_segment(duration=duration_ms).apply_gain(-26)
    layer2 = Sine(165).to_audio_segment(duration=duration_ms).apply_gain(-30)
    mixed = base.overlay(layer).overlay(layer2).fade_in(2000).fade_out(3000)
...
def validate_master(path: str | Path, min_duration: float = 5.0) -> dict:
    ...
    checks = {
        ...
        "duration_ok": probe_duration(path) >= min_duration,
        "has_video": False,
        "has_audio": False,
    }
    if path.exists():
        video = run_ffprobe([
            "-v", "error", "-select_streams", "v:0", "-show_entries", "stream=codec_name", "-of", "csv=p=0", str(path)
        ])
        checks["has_video"] = video.returncode == 0 and "h264" in video.stdout.lower()
        checks["has_audio"] = probe_has_audio(path)
    return {
        "passed": all(checks.values()),
        "checks": checks,
        "duration": probe_duration(path) if path.exists() else 0,
    }


__all__ = [
    "concat_clips", "mix_narration_and_music", "apply_color_grade", "burn_subtitles", "master_encode",
    "generate_ambient_music", "probe_duration", "probe_has_audio", "validate_master",
]
