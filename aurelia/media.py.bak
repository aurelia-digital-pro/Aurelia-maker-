...
from .ffmpeg_util import ffmpeg_binary, ffprobe_binary, run_ffmpeg, run_ffprobe
...
def mix_narration_and_music(
    ...
    if music and Path(music).exists():
        _run([
            ...
            "-i", str(music),
            "-filter_complex",
            "[1:a]volume=1.0[narr];[2:a]volume=0.18[bgm];[narr][bgm]amix=inputs=2:duration=first:dropout_transition=2[aout]",
            "-map", "0:v:0", "-map", "[aout]",
...
def burn_subtitles(
    ...
    _run([
        "-y", "-hide_banner", "-loglevel", "error",
        "-i", str(source),
        "-vf", f"subtitles='{srt_path.as_posix()}':force_style='FontSize=28,PrimaryColour=&HFFFFFF,OutlineColour=&H000000,Outline=2,MarginV=40'",
        "-c:v", "libx264", "-preset", "medium", "-crf", "17",
        "-c:a", "copy",
        "-pix_fmt", "yuv420p",
        str(output_path),
...
def generate_ambient_music(duration_sec: float, output: str | Path) -> Path:
    ...
    mixed = base.overlay(layer).overlay(layer2)
...
def validate_master(path: str | Path, min_duration: float = 5.0) -> dict:
    ...
    if path.exists():
        ...
        checks["has_video"] = result.returncode == 0 and "h264" in result.stdout.lower()
